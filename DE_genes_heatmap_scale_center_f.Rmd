...
#import data
```{r}
Col.1h <- read.csv("C:/Users/LCM/Documents/RNAseq files/differentially expressed gene list/DEgenes.Col.1h.rH.csv")
Col.49h <- read.csv("C:/Users/LCM/Documents/RNAseq files/differentially expressed gene list/DEgenes.Col.49h.rH.csv")
myc234.1h <- read.csv("C:/Users/LCM/Documents/RNAseq files/differentially expressed gene list/DEgenes.myc234.1h.rH.csv")
myc234.49h <- read.csv("C:/Users/LCM/Documents/RNAseq files/differentially expressed gene list/DEgenes.myc234.49h.rH.csv")
myc234.1h.rCol <- read.csv("C:/Users/LCM/Documents/RNAseq files/differentially expressed gene list/DEgenes.myc234.1h.rCol.rH.csv")
myc234.49h.rCol <- read.csv("C:/Users/LCM/Documents/RNAseq files/differentially expressed gene list/DEgenes.myc234.49h.rCol.rH.csv")
...
DEgenes <- join_all(list((Col.1h %>% filter(FDR<=0.01))[2],(Col.49h %>% filter(FDR<=0.01))[2],(myc234.1h %>% filter(FDR<=0.01))[2],(myc234.49h %>% filter(FDR<=0.01))[2], (myc234.1h.rCol %>% filter(FDR<=0.01))[2], (myc234.49h.rCol %>% filter(FDR<=0.01))[2]), by='gene_id', type='full')
...
mdf$value2 <- sqrt(abs(mdf$value))*sign(mdf$value)
...
plotly_IMAGE(final.plot2, width = 1000, height = 500, format = "jpeg", scale = 2,
             out_file = "DEgenes.heatmap.final.version.scale.false.jpeg")
